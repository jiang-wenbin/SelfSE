# -*- coding: utf-8 -*-
"""
Ref: https://github.com/microsoft/DNS-Challenge/blob/master/audiolib.py
@author: chkarada
"""
import os
import numpy as np
...
def normalize_with_peak(audio, peak_level=-2.02):
    '''Normalize the signal to the peak level'''
    audio_peak = max(audio)
    scalar = 10 ** (peak_level / 20) / (audio_peak+EPS)
    audio = audio * scalar
    return audio
...
def audioread(path, sr=None, norm=False, start=0, stop=None, target_level=-25):
